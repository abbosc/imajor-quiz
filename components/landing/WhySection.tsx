...
const reasonIds = ['exploration', 'allInOne', 'safe', 'free', 'noChaos', 'growing'] as const;
...
const reasonColors: Record<string, string> = {
  exploration: 'from-amber-500 to-orange-500',
  allInOne: 'from-blue-500 to-cyan-500',
  safe: 'from-green-500 to-emerald-500',
  free: 'from-purple-500 to-pink-500',
  noChaos: 'from-rose-500 to-red-500',
  growing: 'from-indigo-500 to-violet-500',
};
...
function ReasonCard({ reasonId, t }: { reasonId: string; t: (key: string) => string }) {
  return (
    <motion.div
    ...
    >
      ...
      <motion.div
        className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-gradient-to-br ${reasonColors[reasonId]} flex items-center justify-center text-white mb-4 sm:mb-5 shadow-lg`}
        whileHover={{
          scale: 1.1,
          rotate: [0, -5, 5, 0],
      ...
      <div className="container mx-auto px-6 sm:px-8 lg:px-12 relative z-10">
        ...
        >
          {reasonIds.map((reasonId) => (
            <ReasonCard key={reasonId} reasonId={reasonId} t={t} />
          ))}
        </motion.div>
      </div>
    </section>
