...
const collegeAppGroup: NavGroup = {
  label: 'CollegeApp',
  icon: (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  // Auto-expand CollegeApp if any of its items are active
  useEffect(() => {
    const isCollegeAppActive = collegeAppGroup.items.some(
      item => pathname === item.href || pathname.startsWith(item.href + '/')
    );
    if (isCollegeAppActive) {
      setCollegeAppOpen(true);
    }
  }, [pathname]);

  // Prefetch all dashboard routes on mount for instant navigation
  useEffect(() => {
    const allRoutes = [...topNavItems, ...collegeAppGroup.items, ...secondaryNavItems];
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto scrollbar-hide p-4 space-y-1">
          ...
          <div className="pt-1">
            <button
              onClick={() => setCollegeAppOpen(!collegeAppOpen)}
            ...
            >
              {collegeAppGroup.icon}
              <span className="font-medium">{collegeAppGroup.label}</span>
              <svg
                className={`w-4 h-4 ml-auto transition-transform ${collegeAppOpen ? 'rotate-180' : ''}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div
              className={`overflow-hidden transition-all duration-200 ${
                collegeAppOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
              }`}
            >
              <div className="pt-1 space-y-0.5">
                {collegeAppGroup.items.map((item) => (
        ...
        {/* User section */}
        <div className="p-4 pb-8 lg:pb-4 border-t border-[#E2E8F0]">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-[#64748B] truncate">{profile?.email}</p>
            </div>
          </div>
          <button
            onClick={async () => {
