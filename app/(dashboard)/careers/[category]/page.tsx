...
function CategoryIcon({ slug, color, className = "w-7 h-7" }: { slug: string; color: string; className?: string }) {
...
export default function CategoryPage() {
  ...
  useEffect(() => {
    async function fetchCategory() {
      try {
        const res = await fetch(`/api/careers/categories/${categorySlug}`);
  ...
  if (loading) {
    return (
      <div className="min-h-[80vh]">
        <div className="h-8 w-48 bg-[#F1F5F9] rounded animate-pulse mb-6" />
        <div className="h-12 w-96 bg-[#F1F5F9] rounded animate-pulse mb-4" />
        <div className="h-6 w-72 bg-[#F1F5F9] rounded animate-pulse mb-10" />
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-40 rounded-2xl bg-[#F1F5F9] animate-pulse" />
          ))}
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold text-[#0F172A] mb-2">Category not found</h3>
          <p className="text-[#64748B] mb-6">The category you&apos;re looking for doesn&apos;t exist.</p>
  ...
  const color = category.color || '#FF6B4A';
    ...
    <div className="min-h-[80vh]">
      ...
      >
        <div className="flex items-start gap-4 mb-4">
          ...
          >
            <CategoryIcon slug={category.slug} color={color} className="w-7 h-7" />
          </div>
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-[#0F172A] tracking-tight">
              {category.name}
            </h1>
            <p className="text-[#64748B] mt-1 max-w-xl">
      ...
      {category.majors.length === 0 ? (
        ...
        >
          ...
          <h3 className="text-xl font-semibold text-[#0F172A] mb-2">No majors yet</h3>
          <p className="text-[#64748B]">Majors for this category will appear here once added.</p>
        </motion.div>
      ) : (
        <>
          <motion.p
          ...
          >
            {category.majors.length} {category.majors.length === 1 ? 'major' : 'majors'} available
          </motion.p>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {category.majors.map((major, index) => (
              <motion.div
                key={major.id}
              ...
              >
                <Link href={`/careers/${categorySlug}/${major.slug}`}>
                  <div className="group bg-white rounded-2xl p-5 h-48 border border-[#E2E8F0] hover:border-[#CBD5E1] hover:shadow-md transition-all duration-200 cursor-pointer flex flex-col">
                    ...
                    <div className="mt-auto min-w-0">
                      ...
                      <p className="text-sm text-[#64748B] line-clamp-2 mb-3">
                        {major.description || 'Explore careers in this field'}
                      </p>
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-[#64748B]">
                          {major.careers_count} {major.careers_count === 1 ? 'career' : 'careers'}
                        </span>
                        <svg
